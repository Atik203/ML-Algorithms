...
\begin{lstlisting}[style=mlcode,caption={Load a pre-trained DistilBERT tokenizer (fast, subword/WordPiece based)}]
...
\begin{lstlisting}[style=mlcode,caption={Load a pre-trained sentiment-analysis pipeline (DistilBERT SST-2)}]
...
\begin{lstlisting}[style=mlcode,caption={Load a small causal LM (DistilGPT2) for text generation}]
...
\begin{lstlisting}[style=mlcode,caption={Real dataset: SMS Spam Collection (5,572 labeled messages)}]
...
\begin{lstlisting}[style=mlcode,caption={Fine-tuning loop (2 epochs on 400 real SMS messages)}]
...
\begin{lstlisting}[style=mlcode,caption={Evaluate the fine-tuned model on held-out real SMS messages}]
