...
\begin{codebox}[title={2D real data for visualizing margins and kernels}]
...
\begin{codebox}[title={SVC on a real dataset: Breast Cancer (RBF kernel)}]
...
\begin{codebox}[title={Real 1D regression: Old Faithful geyser}]
...
plt.title(f"Support Vector Regression on Old Faithful (C=10, eps={epsilon_val})", fontsize=14, fontweight='bold')
...
print(f"SVR R2 Score: {r2_score(y_svr, svr_model.predict(X_svr_scaled)):.4f}")
