...
\begin{codebox}[title={Suppress a known sklearn deprecation notice for SVC(probability=True)}]
...
\begin{codebox}[title={Experiment 1: Self-Training with an RBF-kernel SVC}]
...
\begin{codebox}[title={Inspect how self-training converged}]
...
\begin{codebox}[title={Compact comparison table (SVC)}]
