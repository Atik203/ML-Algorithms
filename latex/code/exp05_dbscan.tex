...
\begin{codebox}[title={Load the real USGS earthquake catalogue (global, 2023)}]
...
\begin{codebox}[title={Choosing epsilon from the k-distance graph (haversine metric)}]
...
plt.axhline(y=0.03, color='navy', linestyle='--', label='Selected eps = 0.03 rad (~191 km)')
plt.legend()
plt.tight_layout()
plt.show()
\end{codebox}

\begin{codebox}[title={Fit DBSCAN with the chosen epsilon and MinPts}]
...
plt.title(f"DBSCAN on Global Earthquakes 2023 (eps={eps_selected} rad, {n_clusters_db} clusters)", fontsize=13, fontweight='bold')
...
\begin{codebox}[title={Parameter sensitivity: DBSCAN under several (eps, min\_samples) settings}]
