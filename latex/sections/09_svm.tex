...
\chapter{Experiment 17: Support Vector Machine
(SVM)}\label{experiment-17-support-vector-machine-svm}

\textbf{Category:} Supervised / margin-based learning

\section{1. Objective}\label{objective}

Train SVM classifiers and investigate kernels, C and gamma; also
demonstrate Support Vector Regression with an ε-insensitive tube.
...
\tightlist
\item
  Breast Cancer (569 × 30): two features (mean radius, mean texture) for
  the 2D kernel view and all 30 features for the accuracy benchmark. (b)
  Old Faithful geyser (\passthrough{\lstinline!data/geyser.csv!}, 272
  eruptions) for SVR.
\end{enumerate}
...
SVC maximizes the margin while penalizing violations: min
½\textbar\textbar w\textbar\textbar² + C Σξ\_i. The dual depends only on
inner products K(x\_i, x\_j) (kernel trick): linear, polynomial, RBF. C
controls the violation penalty; γ controls the RBF influence radius. SVR
ignores residuals within ±ε and uses only outside points (support
vectors).
...
\item
  Scale features.
\item
  Train SVC with linear/poly/RBF kernels on the 2D cancer data; plot
  boundaries, margins and support vectors.
\item
  Train the RBF SVC on all 30 features; report accuracy and
  support-vector count.
\item
  Fit SVR (RBF, C=10, ε=0.3) to waiting time → eruption duration; plot
  the fitted curve and ε-tube.
\item
  Report R², RMSE and support-vector share.
\end{enumerate}
...
The 2D kernel figure shows the linear kernel underfitting the real
diagnostic data while poly/RBF produce curved boundaries. The geyser
scatter is bimodal; the RBF SVR smoothly bridges the two modes and the
ε=0.3 tube leaves most residuals unpenalized.

\section{8. Discussion and Conclusion}\label{discussion-and-conclusion}

SVMs give accurate, sparse solutions: 77\% of the cancer training points
could be discarded without changing the classifier. They require scaling
and tuning (C, γ), and kernel methods scale as O(n²), which limits them
...
\tightlist
\item
  \textbf{What is a support vector?} A training point on or inside the
  margin (α \textgreater{} 0) that alone defines the decision boundary.
\item
  \textbf{Effect of C?} Large C penalizes violations strongly → narrow
  margin, possible overfitting; small C allows a wider margin and more
  violations.
\item
  \textbf{Effect of gamma?} Large γ makes each point's influence local →
  wiggly boundary; small γ makes the RBF smoother, approaching a linear
  model.
\item
  \textbf{Why does SVM require careful scaling?} Margins and kernels use
  Euclidean distances; unscaled features with larger ranges dominate
  them and degrade the fit.
\end{itemize}
...
\passthrough{\lstinline!notebooks/09\_support\_vector\_machine.ipynb!}
(executed; results above are its actual output).}

\textbf{Listing 17.1 --- Core numerical and plotting libraries}
...
\textbf{Listing 17.2 --- 2D real data for visualizing margins and
kernels}
...
\textbf{Listing 17.3 --- SVC on a real dataset: Breast Cancer (RBF
kernel)}
...
\textbf{Listing 17.4 --- Real 1D regression: Old Faithful geyser}
