...
% ---------------------------------------------------------------------------
\chapter{Support Vector Machine (SVM)}\label{exp:17}

\runninghead{Experiment 17: Support Vector Machine (SVM)}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{|l|l|r|}
    \hline
\rowcolor{mlblue!8}    \textbf{Experiment} & \textbf{Metric} & \textbf{Value} \\
    \hline
...
\begin{enumerate}
  \item \textbf{What is a support vector?}\par\nobreak A training point on or inside the margin ($\alpha > 0$) that
  alone defines the decision boundary.
  \item \textbf{Effect of $C$?}\par\nobreak Large $C$ penalizes violations strongly (narrow margin, possible
  overfitting); small $C$ allows a wider margin and more violations.
  \item \textbf{Effect of $\gamma$?}\par\nobreak Large $\gamma$ makes each point's influence local (wiggly
  boundary); small $\gamma$ makes the RBF smoother, approaching a linear model.
  \item \textbf{What is the kernel trick?}\par\nobreak Computing inner products in a high-dimensional feature
  space without explicitly mapping the points there.
  \item \textbf{What is the $\varepsilon$-tube in SVR?}\par\nobreak The zero-loss zone around the fitted function;
  only points outside it become support vectors. A wider tube (0.3 vs.\ 0.1) reduced the
  support-vector count with the same $R^2$.
  \item \textbf{Why does SVM require careful scaling?}\par\nobreak Margins and kernels use Euclidean distances;
  unscaled features with larger ranges dominate them and degrade the fit.
\item \textbf{What is the difference between SVC and SVR?}\par\nobreak SVC solves classification by maximizing the margin between classes; SVR solves regression by fitting a tube of width $\varepsilon$ around the target and penalizing only points outside it.
\end{enumerate}
