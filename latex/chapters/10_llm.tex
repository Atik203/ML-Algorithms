...
% ---------------------------------------------------------------------------
\chapter{Large Language Model (LLM) Experiment}\label{exp:18}

\runninghead{Experiment 18: Large Language Model (LLM) Experiment}
...
\begin{enumerate}
  \item \textbf{What is tokenization?}\par\nobreak Splitting text into subword units that map to integer
  ...
  \item \textbf{Pretraining vs fine-tuning vs prompting?}\par\nobreak Pretraining learns general language
  ...
  \item \textbf{Why is LLM output not automatically ground truth?}\par\nobreak The model generates the most
  ...
  \item \textbf{How can prompt wording affect results?}\par\nobreak Meaning, tone, format and label wording all
  change the conditional distribution the model samples from, so prompts must be fixed and
  reported.
\item \textbf{Why must the test messages be separated before fine-tuning?}\par\nobreak Fine-tuning on data that later serves as the test set leaks the evaluation targets into training; the held-out split must be created first so the reported accuracy stays honest.
\end{enumerate}
