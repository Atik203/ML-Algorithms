...
% ---------------------------------------------------------------------------
\chapter{Experiment 8: Random Forest Regression (RFR)}\label{exp:08}
...
\begin{lstlisting}[style=mlcode,caption={Random Forest Regression with out-of-bag validation}]
...
\begin{description}
  \item[Why does a forest reduce variance?] Averaging $B$ approximately uncorrelated trees
  gives $\rho\sigma^2 + (1-\rho)\sigma^2/B$; as $B$ grows the second term vanishes, so
  predictions are more stable than a single tree's.
  \item[What is bagging?] Bootstrap aggregating: each model is trained on a bootstrap sample
  of the data and the predictions are combined (average for regression, vote for
  classification).
  \item[Why can feature importance be misleading?] Impurity-based importance is biased toward
...
\input{code/exp08_rfr}

% ---------------------------------------------------------------------------
\chapter{Experiment 9: Random Forest Classification (RFC)}\label{exp:09}
...
\section{Reference Implementation}
\begin{lstlisting}[style=mlcode,caption={Random Forest Classification with stratified evaluation}]
...
\section{Viva / Discussion Questions}
\begin{description}
  \item[RFR vs RFC?] Regression averages the trees' numeric outputs; classification takes a
  majority vote (and averages class probabilities for scoring).
  \item[What is majority voting?] Each tree votes for one class and the most frequent class
  becomes the ensemble prediction; soft voting averages the probabilities instead.
  \item[How can class imbalance affect accuracy?] With a rare positive class, a model can
  predict the majority class everywhere and still show high accuracy; precision, recall, F1 and
  AUC expose the failure.
\end{description}

\section{Complete Code Listing}
\input{code/exp09_rfc}

% ---------------------------------------------------------------------------
\chapter{Experiment 10: XGBoost}\label{exp:10}
...
\section{Reference Implementation}
\begin{lstlisting}[style=mlcode,caption={XGBoost classification on the heart data}]
...
\section{Viva / Discussion Questions}
\begin{description}
  \item[Bagging vs boosting?] Bagging trains models in parallel on bootstrap samples and
  ...
  \item[What does the learning rate do?] It shrinks each tree's contribution; smaller values
  need more trees but usually generalize better, larger values fit faster and can overfit.
  \item[Why can boosting overfit?] Later trees keep fitting remaining errors, including label
  noise, so the ensemble can become too complex; depth limits, regularization, subsampling and
  early stopping control this.
\end{description}

\section{Complete Code Listing}
\input{code/exp10_xgboost}

% ---------------------------------------------------------------------------
\chapter{Experiment 11: AdaBoost}\label{exp:11}
...
\section{Reference Implementation}
\begin{lstlisting}[style=mlcode,caption={AdaBoost classification}]
...
\section{Viva / Discussion Questions}
\begin{description}
  \item[Why use weak learners?] Individually they are simple and low-variance; boosting
  combines many of them so the ensemble can fit complex boundaries while each step stays
  simple.
  \item[How does AdaBoost focus on difficult examples?] After each round, weights of
  misclassified samples increase, so the next learner optimizes a distribution that emphasizes
  current errors.
  \item[What is the effect of noisy labels?] Mislabeled points look ``hard'', receive
  ever-larger weights and can dominate training, degrading accuracy --- AdaBoost is known to be
  noise-sensitive.
\end{description}

\section{Complete Code Listing}
\input{code/exp11_adaboost}

% ---------------------------------------------------------------------------
\chapter{Experiment 12: CatBoost}\label{exp:12}
...
\section{Reference Implementation}
\begin{lstlisting}[style=mlcode,caption={CatBoost with native categorical features}]
...
\section{Viva / Discussion Questions}
\begin{description}
  \item[How does CatBoost treat categorical features?] It computes ordered target statistics
  (the mean target per category on random permutations, with prior smoothing) rather than
  one-hot vectors.
  \item[Why should categories not be encoded using target leakage?] Plain target encoding uses
  ...
  \item[CatBoost vs ordinary one-hot encoding?] One-hot explodes dimensionality for
