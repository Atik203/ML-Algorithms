...
\begin{enumerate}
  \item \textbf{Why does an RNN have memory?}\par\nobreak Its hidden state is carried across time steps, so each
  output depends on previous inputs, not only the current one.
  \item \textbf{What is a hidden state?}\par\nobreak The internal vector summarizing the sequence seen so far; it
  is updated at every step and used to produce outputs.
  \item \textbf{Why are LSTM/GRU often preferred for long dependencies?}\par\nobreak Their gates control what is
  ...
  \item \textbf{Why is the train/test split chronological?}\par\nobreak Shuffling sequences would leak future
  values into training and produce an optimistic, invalid evaluation.
\item \textbf{How do the gates of an LSTM control information flow?}\par\nobreak Forget, input and output gates multiply the cell and hidden states with values between 0 and 1, so the network learns what to keep, write and expose over long sequences.
\end{enumerate}
