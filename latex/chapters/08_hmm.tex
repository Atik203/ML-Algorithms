...
\begin{enumerate}
  \item \textbf{What is hidden in an HMM?}\par\nobreak The state sequence: we observe emissions (returns) but not
  the underlying regime that generated them.
  \item \textbf{Viterbi vs forward algorithm?}\par\nobreak Viterbi returns the single most likely state path (a
  maximum over paths); the forward algorithm computes the total probability $P(X \mid \lambda)$
  by summing over all paths.
  \item \textbf{What is the Markov assumption?}\par\nobreak The future state depends on the past only through the
  present state: $P(z_t \mid z_{1:t-1}) = P(z_t \mid z_{t-1})$.
  \item \textbf{Why sort the states by volatility?}\par\nobreak So that regime labels (Bull/Sideways/Bear) are
  stable and interpretable across runs.
  \item \textbf{Why model returns and not prices?}\par\nobreak Returns are approximately stationary; prices trend
  and violate the Gaussian emission assumption.
\item \textbf{How do you choose the number of hidden states?}\par\nobreak Compare candidate models by held-out likelihood (or BIC/AIC), check whether the decoded states are interpretable, and avoid states that merely fit noise.
\end{enumerate}
