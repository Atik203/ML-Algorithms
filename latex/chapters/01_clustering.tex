...
\begin{codebox}[title={Selecting K, then fitting the final K-means model}]
for k in range(2, 11):
...
\section{Reference Implementation}
\begin{codebox}[title={K-Means++ followed by the outlier-distance check}]
...
\section{Reference Implementation}
\begin{codebox}[title={Ward linkage, dendrogram and the final agglomerative model}]
...
\section{Reference Implementation}
\begin{codebox}[title={Core alternating updates of Fuzzy C-means}]
Um = U ** self.m
