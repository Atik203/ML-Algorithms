...
% ---------------------------------------------------------------------------
\chapter{Multilayer Perceptron (MLP)}\label{exp:13}

\runninghead{Experiment 13: Multilayer Perceptron (MLP)}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{|l|r|l|}
    \hline
\rowcolor{mlblue!8}    \textbf{Model} & \textbf{Test accuracy} & \textbf{Notes} \\
    \hline
...
\begin{enumerate}
  \item \textbf{Why is scaling important for MLP?}\par\nobreak Gradient descent converges poorly when input
  features have very different scales; standardization equalizes their influence and stabilizes
  the loss surface.
  \item \textbf{What is backpropagation?}\par\nobreak Reverse-mode application of the chain rule that computes the
  gradient of the loss with respect to every weight, layer by layer.
  \item \textbf{What causes vanishing/exploding gradients?}\par\nobreak Repeated multiplication of small (or
  large) Jacobians through many layers; ReLU, BatchNorm, careful initialization and residual
  connections mitigate it.
  \item \textbf{Why is the PyTorch model better here?}\par\nobreak BatchNorm and Dropout plus a longer, scheduled
  training; the scikit-learn model stops early at iteration 15.
\item \textbf{Why does early stopping act as a regularizer?}\par\nobreak It stops training before the network starts fitting noise; the validation error is at its minimum at that point, which limits effective model complexity.
\end{enumerate}
