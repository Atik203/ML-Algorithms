...
% ---------------------------------------------------------------------------
\chapter{Experiment 5: DBSCAN}\label{exp:05}
...
\begin{lstlisting}[style=mlcode,caption={$\varepsilon$ from the $k$-distance knee, then DBSCAN with the haversine metric}]
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}lrrr@{}}
    \toprule
    \textbf{$\varepsilon$ (rad)} & \textbf{approx.\ km} & \textbf{clusters} & \textbf{noise} \\
    \midrule
    0.02 & 127 & 70 & 21.2\% \\
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}lrrl@{}}
    \toprule
    \textbf{Cluster} & \textbf{Events} & \textbf{Center lat/lon} & \textbf{Dominant region} \\
    \midrule
...
% ---------------------------------------------------------------------------
\chapter{Experiment 6: HDBSCAN}\label{exp:06}
...
\section{Reference Implementation}
\begin{lstlisting}[style=mlcode,caption={HDBSCAN on the earthquake data (no global epsilon)}]
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}lrrrr@{}}
    \toprule
    \textbf{Algorithm} & \textbf{Clusters} & \textbf{Noise} & \textbf{Largest cluster} & \textbf{Silhouette} \\
    \midrule
