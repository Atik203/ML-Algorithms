...
\begin{enumerate}
  \item \textbf{Why does DBSCAN not require $K$?}\par\nobreak Clusters are grown from density-connected core
  points, so their number and shape emerge from the data instead of being fixed in advance.
  \item \textbf{What does label $-1$ mean?}\par\nobreak The point is neither a core nor a border point --- it lies
  in a sparse region and is treated as noise/outlier (here: isolated earthquakes).
  \item \textbf{Why can DBSCAN struggle with varying densities?}\par\nobreak A single $\varepsilon$ that is small
  enough for a dense cluster fragments a sparse one, while one large enough for the sparse
  cluster merges the dense ones.
  \item \textbf{Why is haversine used instead of Euclidean distance?}\par\nobreak Latitude/longitude are
  spherical coordinates; haversine gives true great-circle distances (radian inputs,
  $\varepsilon$ in radians).
  \item \textbf{How is $\varepsilon$ selected?}\par\nobreak From the knee of the $k$-distance graph with
...
\section{Viva / Discussion Questions}
\begin{enumerate}
  \item \textbf{What is hierarchical about HDBSCAN?}\par\nobreak It constructs a full hierarchy of density-based
  ...
  \item \textbf{What does membership probability mean?}\par\nobreak A per-point confidence that it belongs to its
  ...
  \item \textbf{Why is HDBSCAN useful for variable-density data?}\par\nobreak Clusters are selected by stability
  across density levels rather than by one global $\varepsilon$, so dense and sparse clusters
  can coexist.
  \item \textbf{What does \texttt{min\_cluster\_size} control?}\par\nobreak The smallest group that may become a
  cluster; increasing it reduces the number of small clusters, decreasing it fragments the map.
\item \textbf{What does cluster persistence (stability) mean in HDBSCAN?}\par\nobreak It measures how long a cluster survives across the density hierarchy; clusters that persist over a wide range of density levels are selected, which is why HDBSCAN handles variable densities.
\end{enumerate}
